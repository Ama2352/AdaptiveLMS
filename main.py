...
class ProgressResponse(BaseModel):
    ...
    progress_details: List[dict] # {concept_id, is_mastered, current_elo}
...
def get_student_progress_logic(student_id: str, conn):
    ...
    # Find "Current Topic" (First unmastered? Or use next-question logic?)
    # Simple heuristic: First unmastered concept in order? 
    # Or just "Adaptive Learning"
    
    current_topic = "Adaptive Learning Path"
    for r in rows:
        if not r['is_mastered']:
            current_topic = f"{r['chapter_name']}: {r['concept_name']}"
            break
